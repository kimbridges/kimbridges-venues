...
create_spread_pdf <- function(image_path, 
  ...
  exif_data <- exif_read(image_path, tags = c("Caption-Abstract", "Sub-location", 
                                              "Province-State", "DateTimeOriginal"))
  ...
  state <- get_field(exif_data, 'Province-State')
  date_raw <- get_field(exif_data, 'DateTimeOriginal')
  
  # Parse date to 'Month Year' format
  date_formatted <- ''
  if (nchar(date_raw) > 0) {
    date_parsed <- tryCatch(
      as.POSIXct(date_raw, format = '%Y:%m:%d %H:%M:%S'),
      error = function(e) NULL
    )
    if (!is.null(date_parsed)) {
      date_formatted <- format(date_parsed, '%B %Y')
    }
  }
  ...
  # Build line 2 from location/date (may be empty)
  location_parts <- c(location, state, date_formatted)
