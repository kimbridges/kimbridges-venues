## --- Photo Book (refactor) ---
## Kim-friendly style: tabular config via readr, clear steps, optional map
...
parse_story_filename <- function(fname) {
  ...
  if (is.na(m[1,1])) {
    list(order = Inf, stem = base)   ## no numeric prefix => put at end, stem is whole
  } else {
...
## 4. Discover and order stories by numeric prefix
...
for (i in seq_len(nrow(story_tbl))) {
  ...
  story_stem <- story_tbl$stem[i]  ## used to match EXIF Event
...
## 7. Verification table page (gt)
if (length(verification_rows) > 0) {
  message("\nGenerating verification table...")
  verification_df <- list_rbind(verification_rows)
  
  ## Use Kim’s gt mods if sourced; else a light default
  table_pdf_path <- "temp_verification_table.pdf"
  
  if (exists("gt_table")) {
    ## If gt_mods.R defined an object named gt_table expecting verification_df,
    ## re-render it here by building from the function or snippet.
    ## To keep things simple, rebuild with same styles:
...
if (make_map) {
  ...
  # Write the used-image list so make_map can limit itself
  ...
  ## Provide config to the map via env vars so it doesn’t need its own config parsing
