...
#     independent of the DEM's native resolution. Fetch a fine DEM, aggregate
#     to the analysis scale, compute slope/aspect there; measure roughness as
#     elevation SD within a fixed radius. Results become comparable + honest.
#
# SOURCES (source = ):
#   "aws"    default. elevatr AWS Terrain Tiles: free, no key, global, and
#            automatically finer over the US (3DEP/lidar-blended). z sets the
#            native tile resolution (z14 ~ 4-10 m).
#   "google" legacy 2020 3x3 method, kept as a fallback (needs a billed key).
#
# Deliberately dropped vs v1: bare TRI (scale-ambiguous) -> roughness_sd at a
# stated radius; the N-S/E-W grid-label bug (already fixed in v1) stays fixed.
# -----------------------------------------------------------------------------

library(terra)
library(httr)
library(jsonlite)
# elevatr + sf are loaded on demand by the "aws" backend.
...
# terra D8 flow-direction code -> compass label.
.flowdir_compass <- function(code) {
  c(`1`="E", `2`="SE", `4`="S", `8`="SW",
    `16`="W", `32`="NW", `64`="N", `128`="NE")[[as.character(code)]]
}
...
.dem_aws <- function(lon, lat, z = 14, aoi_m = 300) {
  if (!requireNamespace("elevatr", quietly = TRUE) ||
    ...
    stop("source = 'aws' needs the 'elevatr' and 'sf' packages.")
...
# Hillshade of the DEM patch with the collection point marked (fine-res "site
# view"). Works for the aws source (real DEM); for google the 3x3 is too coarse.
...
# Bedrock unit, lithology and age at a point — Macrostrat (global, keyless).
site_geology <- function(lat, lon) {
  ...
  list(unit      = toString(d$name),
       lithology = lith,
       age_ma    = sprintf("%g-%g", min(d$t_age, na.rm = TRUE), max(d$b_age, na.rm = TRUE)),
       source    = "Macrostrat")
}
...
# Both substrate layers, with ready-to-use label sentences.
...
# OpenStreetMap Nominatim: free, keyless (no paid Google API). Policy limit
# ~1 request/second, so batch/cache for bulk (e.g. legacy enrichment).
site_place <- function(lat, lon, zoom = 16) {
  u <- sprintf("https://nominatim.openstreetmap.org/reverse?lat=%f&lon=%f&format=jsonv2&zoom=%d&addressdetails=1",
               lat, lon, zoom)
  r <- tryCatch(httr::GET(u, httr::add_headers(`User-Agent` = "specimen-labels/0.1 (research use)"),
  ...
  # Compose from the RELIABLE administrative levels only. Skip OSM's `city`
  # field and the postcode, which are erratic in dense urban areas (verified:
  # they returned a wrong district + ZIP here). The coordinates remain the
  # authoritative locality; the place name is a human-readable convenience.
...
# --- example -----------------------------------------------------------------
# source("site_extract.R")
# koko <- site_extract("Koko Crater", 21.28164, -157.68687)          # aws default
# cat(koko$sentences, sep = "\n")
# site_hillshade(koko)
# sub  <- site_substrate(21.28164, -157.68687)   # bedrock (Macrostrat) + soil (SSURGO/SoilGrids)
