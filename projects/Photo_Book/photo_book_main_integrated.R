...
for (i in seq_len(nrow(story_tbl))) {
  ...
  for (img in image_files) {
    exif <- safe_read_exif(img, c("Event","Description","Location","Sub-location","City","GPSLatitude","GPSLongitude"))
    ...
    ## Get Location (try Sub-location first, then Location)
    location_val <- if ("Sub-location" %in% names(exif) && !is.null(exif$`Sub-location`)) {
      exif$`Sub-location`
    } else {
      exif$Location %||% "NA"
    }
...
if (length(verification_rows) > 0) {
  ...
  ## Use custom gt_mods function if available, otherwise default styling
  if (exists("create_verification_table", mode = "function")) {
    gt_tbl <- create_verification_table(verification_df)
  } else {
