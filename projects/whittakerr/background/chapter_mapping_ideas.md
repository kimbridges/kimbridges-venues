...
## Status
Both chapters drafted. Build a Map drafted 2026-05-22
(`build_a_map.qmd`); Beyond a Map drafted 2026-05-23
(`beyond_a_map.qmd`). This file served as the shared ideation
scaffold for both and is now of historical interest only —
the chapters have absorbed its content.

Two departures from the plan below are worth recording.
(1) Beyond a Map was built on Kenya, not on the Oahu and
Oregon worked examples this file anticipated: an unfamiliar
country makes the chapter's anchor argument land as true
rather than merely stated, and Mount Kenya at 30 arcseconds
carries the smoothing artifact. (2) Thread 5 (boundary
representation — a `border` option with crisp / soft / none /
uncertainty variants) was dropped: it was never built into
the toolkit, and the chapter teaches only what exists.

The original ideation, including the Oahu/Oregon example plan
and Thread 5, is preserved below as written.
