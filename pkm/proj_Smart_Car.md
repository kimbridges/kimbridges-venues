...
**★★★ FOURTEEN PIECES DRAFTED. Foreword and chapters 1-13 exist as prose -- about 27,000 words. 16 chapters render, 0 error strings, 0 unresolved cross-references (2026-09-01).** ★ Kim has read and approved ch.3, ch.4, ch.5 and ch.12; **ch.6-ch.11 and ch.13 are drafted but unread by him.** **Only ch.14 `what_the_record_cannot_say.qmd` remains.** Per-chapter detail lives in `session_log.md`. The notes below this line describe ch.4 and earlier and are kept as written.
...
- **`book/data/na_states.rds` -- NEW 2026-09-01.** Cached North America state/province geometry (137 KB, `rnaturalearth` simplified to 3 km, 51 US + 13 CA, Alaska included). ⛔ **`ne_states()` HUNG THE R BRIDGE for several minutes on a network call.** `fig_driving()` reads this file and must never call the network at build time.
