...
- **`book/R/stops_data.R` + `book/R/fig_stops.R` -- NEW 2026-09-01.** The fuel-stop POINT map: what a choropleth cannot show, which is **cadence**. Geocoding is offline and reproducible from three sources -- the project gazetteer, the 8 coordinate pairs in `TwoRed_gazetteer_errata.csv` (**keyed both by the corrected name AND by the misspelling the fuel log carries**, worth +2.4 points on its own), and `maps::` city tables with `world.cities` tagged to a state by point-in-polygon against `data/na_states.rds` (cached at `data/wc_tagged.rds`). ⛔ **No network geocoder in a figure function.** **Coverage: TwoRed 256/294 fills (87.1%); Creamsicle 69/173 (39.9%).** ⛔ **Creamsicle's map is NOT drawn at that rate** -- see `deferred.md`.
