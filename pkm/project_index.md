...
**★★★ CROSS-REFERENCES ARE GENERATED NOW, AND THE ROOT WAS THE FOREWORD.** `index.qmd` (*Foreword*) and `afterword.qmd` were being NUMBERED, so the printed book called `small_cars` Chapter 2 while everyone called it ch.1. Both `{.unnumbered}`; every heading carries `{#sec-...}`; prose uses `@sec-name` / `chapter -@sec-name`. **Printed numbering now runs 1-14 exactly as the project speaks it**, two references silently corrected themselves (9->10, 13->14), and they are hyperlinks. **Finding 050.** ⚠ Positional phrases (*"the next chapter"*) stay hand-checked on any reorder -- four lines, listed in `deferred.md`.
...
**(1) I told Kim Finding 033 would not reproduce and asked him to rule on amending it. He ruled amend; then I read 033 and my report was wrong.** 033 states a **substitution-rate** result, and it reproduces and SHARPENS: **0/8 at 15.6% (was 13.3%), with 166 control substitutions and ZERO repairs** (the original had one stray, which failed to recur). **What did not reproduce was a PARAPHRASE in the chapter scaffold -- and that was the version queued for print. Finding 051:** notes summarise notes here, every hop is lossy, the last hop reaches the page, **and the tell is the same as 049 and 050 -- the build succeeds.** Rule: **keep the generator, not the result.**
...
**NEXT: Kim reads ch.6 through ch.9 and the two new sections. Ch.10 `knowing_a_number_is_wrong.qmd` is ch.9's sequel -- re-derive its scaffold numbers per Finding 051. Nothing is blocking.**
<!-- /active-focus -->
