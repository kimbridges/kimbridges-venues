...
**Active Focus (2026-09-01, close): "Smart_Car" -- CH.8 AND CH.9 DRAFTED; CHAPTER NUMBERS NOW MATCH THE WAY WE TALK; AND TWO OF MY OWN CLAIMS WERE CAUGHT ON THE WAY TO THE PAGE.**

**Foreword + ch.1-ch.9 stand, about 20,600 words. 16 chapters render, 0 error strings, 0 unresolved references.** Detail in `session_log.md` 2026-09-01; Kim's accounts and rulings verbatim in `logs/proj_Smart_Car_log.md`.

**★★★ CROSS-REFERENCES ARE GENERATED NOW (Finding 050), AND THE ROOT WAS THAT THE FOREWORD WAS BEING NUMBERED.** `index.qmd` and `afterword.qmd` are `{.unnumbered}`; every heading carries `{#sec-...}`; **printed numbering runs 1-14 exactly as the project speaks it**, two references corrected themselves, and they are hyperlinks. ⚠ Positional phrases stay hand-checked on any reorder (four lines, listed in `deferred.md`).

**★★★ CH.8 `what_driving_cost.qmd` -- the hinge, ~1,250 words.** The record cannot see the driver, **and Kim's own correction proves it**: the prairie days he remembers as the worst of sixteen years run a median 44.5 mph against 40.5 for the record, because *"I couldn't go slow, the big vehicles kept moving at highway speeds."* **The speed IS the stress.** ★★ **Now a TRIPLE** -- fear 47.3, tedium 40.3, pleasure 48.8, all at or above the median. ★★★ **Creamsicle turns it into a measurement:** 84 legs, gross 43.5 vs net 51.0, **a 7.5 mph gap**, so every gross speed in the book is a ceiling. The extreme: `WI Dells -> Madison, 50.3 mi, 5 h, 4.5 stopped`, comment **Cranes**.

**★★★ CH.9 `sixteen_years_of_paper.qmd` -- ~2,360 words, four tables, THE BOOK'S FIRST PHOTOGRAPHIC FIGURE** (a real sheet, Frostburg-3 June 2012, from the scan). **Derived: 20 files, 67 pages** -- the PKM's recurring "22 scans" is a batch count and is wrong. **The 0/8 test runs LIVE off the source workbook**, never edited: 292 rows, **23 fail the pump identity, 8->0 repairs 6, the mirror 1, six controls 1.**

**★★★ TWO OF MY CLAIMS WERE WRONG AND BOTH WERE CAUGHT BEFORE PRINT.**

**(1) I told Kim Finding 033 would not reproduce and he ruled "amend"; then I read 033 and my report was wrong.** It states a **substitution-rate** result and it reproduces and SHARPENS (0/8 at **15.6%**, was 13.3%; **166 control substitutions, zero repairs**). **What failed was a PARAPHRASE in the chapter scaffold, and that was the version queued for print. Finding 051: keep the generator, not the result.**

**(2) ⚠ KIM CORRECTED THE 0/8 MECHANISM: it was never his handwriting, it was the PRINTED RECEIPTS** -- small type, little ink, **a dot in the middle of the zero**, read in a car with the light going. **033's attribution corrected in place; no number moved.** But: **the receipt is the witness and receipts were binned**, so the settling document usually no longer exists; the mechanism belongs to **no writer**, which matters because **there are TWO HANDS in this record**; and **the same document is cause and cure** (surviving receipts print the odometer). ★★★ **A shape in the data points at a PROCESS, not a PERSON. Ask whose hand, then ask whose printer.**

**★★ THE CIRCLED MARK ON THE FIGURE IS A HAPPY FACE — Nancy's birthday, 9 June, the day she joined and the log passes into her hand.** Kim right-handed, Nancy left-handed and an MD. **The page proves the handover by itself:** dates read `6/1`, `6/8`, then **carry the year from 6/9 on.** A second writer shows up as a change of FORMAT, not penmanship. ⚠ Kim says he collected her in Atlanta; **the 2012 trip has no Atlanta** (6/9 is Streetsboro OH -> Jackson MI, 268 mi against ~215 direct, out 09:21). Asked; not printed.

**⚠ Voice: fifth and sixth chapters running where the count caught what the instinct did not.** Also **em dashes had crept into R string literals inside a table -- the sweep must cover code strings.**

**Queued, triggers already fired:** split `proj_Smart_Car.md` (57+ KB against 45); decide about `session_log.md` (281 KB, not splittable).

**★★★ CH.10 `knowing_a_number_is_wrong.qmd` DRAFTED (~1,800 w, two tables, `@fig-audit`) — AND THE AUDIT'S THRESHOLD IS NOW DERIVED, NOT CHOSEN.** Finding 051 applied first and **three scaffold numbers had drifted in a day** (Creamsicle "143 of 170" is really 163; "3 errata, all resolved" is really 5, three of them still hypotheses). **That is the ordinary rate at which hand-copied numbers rot.** ★★★ **The flat "under half a cent" is replaced by a bound implied by the recorded precision:** `0.005 + gallons*0.0005 + ppg*0.0005`, **0.79-1.11c**. TwoRed **281/294** inside their own bound, Creamsicle **165/169** US fills -- same count as before, now with a reason. ★★★ **The finding is the GAP: largest arithmetic residual 0.76c, smallest error 2.93c, a factor of 3.9 with NOTHING in between.** No row needs a judgment call, which is the difference between an instrument and an opinion. ★ **A fourth Creamsicle failure (Port Angeles, 1.17c over a 0.97c bound) is left deliberately unexplained** as the place the instrument runs out. ⚠ Do not give it an erratum. Also: the seven instruments as **what each tests and what each CANNOT see**; contiguity+arithmetic (*the row that balances is the source, the row that fails is the copy*) worked through Oxnard; **six of seven take the date as GIVEN, so none could see a wrong date -- sorting by a field cannot reveal an error in that field**; and geography as **the only instrument that RECOVERS** (15 impossible segments = 4 ferries + 4 gazetteer + 7 suspect odometers, plus Delaware by running it backwards). ⚠ **Instrument 6 renamed to the DOTTED ZERO on the printed receipt** per Kim's correction.

**Foreword + ch.1-ch.10 stand. 16 chapters render, 0 errors, 0 unresolved references.**

**NEXT: Kim reads ch.6 through ch.10 and the two new sections in ch.1 and ch.2. Ch.11 `what_the_car_cost.qmd` is next — re-derive its scaffold numbers first; two of them are known stale.**
<!-- /active-focus -->
