...
**Active Focus (2026-09-01, close): "Smart_Car" -- CH.8 THROUGH CH.12 DRAFTED IN ONE DAY; CHAPTER NUMBERS FIXED; AND THE SCAFFOLDS ARE PROVED TO ROT.**
...
**★★★ FINDING 051 -- KEEP THE GENERATOR, NOT THE RESULT -- EARNED ITS KEEP EIGHT TIMES IN ONE DAY.** Written after a scaffold's PARAPHRASE of Finding 033 turned out to be the version queued for print (033 itself reproduces and **sharpens**). **Then the ch.10, ch.11 and ch.12 scaffolds yielded seven more drifts**, the last two being median leg 251/166 mi where the truth is **305 and 273.7**, and "log distance ~95% of odometer" where the median ratio is **1.00**. **Roughly half of every hand-copied scaffold is wrong. Re-derive before drafting, every time.** ★ Corollary learned today: **a round phrase that feels true is the same defect** -- caught "nine days in ten" in my own ch.12 draft and replaced it with the 49 legs that are derivable.

**CH.8 the hinge** -- the record cannot see the driver, and **Kim's own correction proves it** (*"I couldn't go slow, the big vehicles kept moving at highway speeds"*: the speed IS the stress). **A triple:** fear 47.3, tedium 40.3, pleasure 48.8, all at or above the 40.5 median.

**CH.9 sixteen years of paper** -- the book's first photographic figure, **20 files / 67 pages** derived. ⚠ **Kim corrected the 0/8 mechanism: never his handwriting, it was the PRINTED RECEIPTS** -- small type, little ink, a dot in the zero. **033 corrected in place; no number moved.** ★★★ **A shape in the data points at a PROCESS, not a PERSON. Ask whose hand, then ask whose printer.** ★★ **Kim gave it its ending: the SCANNING EXPERIMENT.** Typed trip log **7 rows -> 139 legs**, and **the scans AUDIT the typing**. *Data has no value while it sits in a log; scanning removes the friction that keeps it there.*

**CH.10 the audit** -- ★★★ **the tolerance is DERIVED, not chosen:** `0.005 + gallons*0.0005 + ppg*0.0005`. **The finding is the GAP: largest arithmetic residual 0.76c, smallest error 2.93c, nothing in between**, so no judgment call is ever required. **Six of the seven instruments take the date as GIVEN.** ⚠ Port Angeles left deliberately unexplained.

**CH.11 what the car cost** -- ★★★ **the two cars are 39.25 and 39.66 mpg over their lives. Four tenths apart**, across sixteen years and two coasts. **Creamsicle costs 26% more per mile and fuel price accounts for 28% of it** (an identity, not a model). **Tank length is the only strong driver: +4.61 mpg per 100 miles. No decline with age.** ⚠ Every price in the book is a PREMIUM price (no grade column).
...
**⚠ Voice: the count caught the density on every chapter today.** Em dashes now also appear in **R string literals** -- the sweep must cover code, not just prose. ⚠ Two figure captions have now been wrong about their own figure until viewed; **look at the PNG before shipping the caption.**

**NEXT: Kim rules on the pair-lesson framing in ch.12's new ending (see `deferred.md`). Ch.6-ch.12 are drafted and unread by him. Then ch.13 `two_constellations.qmd` -- re-derive its scaffold numbers first. Nothing is blocking.**
<!-- /active-focus -->
