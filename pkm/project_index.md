...
**Active Focus (2026-08-31, close): "Smart_Car" -- CH.5 IS DRAFTED, AND THE BOOK RENDERS END TO END FOR THE FIRST TIME.**

**Six pieces stand: Foreword, ch.1, ch.2, ch.3, ch.4, ch.5 -- about 14,200 words.** Kim gave four accounts and reads ch.5 next.

**★★★ THE SCAFFOLD'S EIGHT WAS MISSING THE LONGEST DAY. 2016-06-19 Richmond MO -> Denver, 640 mi, 06:57-16:53, 58.6 mph gross, three fills across Kansas.** Kim's reason inverts the obvious one: **Mount Evans is the next morning, and it was NOT the motivation.** His brother lives in Richmond; the 640 was to get a dull stretch of highway behind him, and what he wanted was I-70 through the Colorado mountains and the road alongside the Colorado River. **The longest day in sixteen years is impatience written down.** ★ **MOUNT EVANS IS NOW MOUNT BLUE SKY** (Kim) -- applied to ch.4 in two places and re-rendered.

**★★★ THE BELLINGHAM CONFLICT IS SETTLED, BY A NUMBER NOBODY HAD TRANSCRIBED.** I recommended the sheet's 06:49 **before reading the collected stories** and had to walk it back; his account names *early afternoon at the dock* specifically. **`TwoRed_fuel_timetemp.csv` held 3 rows for 2013 against 40 for 2016** -- the Alaska trip's fill times had never been entered. 22 rows transcribed from `2013_TwoRed_Fuel_Trip_Logs_Alaska.pdf`, every odometer key gated against the fuel log on the same date, **0 unmatched, 0 date mismatches.** The day then splits: **122 mi in 6.05 h (20.2 mph), then 48.2, then 52.9.** Four hours unaccounted for between Bellingham and Tacoma. **The sheet recorded when the car came off the boat; Kim remembers when the driving started.** ⚠ **Kim has not yet confirmed this reading.** ★★ **The general form: a conflict between two records was only a conflict because a third had never been read.**

**★★ CARSON CITY WAS SMOKE, AND THE ROUTE CORROBORATES IT UNPROMPTED.** Sierra fires over US-395; he could not stop short as intended. The day's two fills are **Kramer Junction 10:40 and Bridgeport 16:34, both 395 towns**, with 77 mi still to run -- from the same new transcription. **SEDONA 2015-07-07, 575 mi, out at 04:05, ends at UNDER 1 MPH** on an unpaved road behind a friend's 4WD, rocks and no way to turn back. The second-longest day in the book, part of it at walking pace.

**★★ TWO PRE-EXISTING RENDER BREAKS FIXED, NEITHER IN CH.5, BOTH THE SAME CLASS.** `small_cars.qmd` (`boot_w`) and `is_it_safe.qmd` (`n_nar`): **inline `r` code reading a variable defined in a chunk LOWER in the file.** Introduced by refactors into `book_setup.R` that renamed what the chapters read. **Invisible to a single-chapter render and to every check except a full-book one.** Now **15/15 chapters render, 0 error strings.**

**Ch.5 `long_days.qmd` -- ~2,660 words, three tables, no figure.** Spine: **not one of the long days was about driving.** A brother, a wrong turn, an empty stretch of Montana, a ferry that did the packing, friends in Las Vegas, a state still needed, a fire, a mother at 104. New `book/R/long_days_data.R`. **Voice rule checked BEFORE showing Kim: 40 contractions, 0 em dashes.**

**NEXT: Kim reads ch.5, then ch.6 `the_roads.qmd`** -- whose deferred TASK needs him to NAME the tree-tunnel stretches, not have them inferred. **Nothing is blocking.**
<!-- /active-focus -->
