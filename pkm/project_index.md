...
**CH.10 the audit** -- ★★★ **the tolerance is DERIVED, not chosen:** `0.005 + gallons*0.0005 + ppg*0.0005`. **The finding is the GAP: largest arithmetic residual 0.76c, smallest error 2.93c, nothing in between**, so no judgment call is ever required. Seven instruments as *what each tests and what each CANNOT see*; **six of seven take the date as GIVEN.** ⚠ Port Angeles left deliberately unexplained.
...
**NEXT: Kim reads ch.12, and ch.6-ch.11 plus the new sections in ch.1 and ch.2 are still unread by him. Then ch.13 `two_constellations.qmd` -- re-derive its scaffold numbers first. Nothing is blocking.**
<!-- /active-focus -->
