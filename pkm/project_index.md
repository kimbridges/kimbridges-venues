...
**Active Focus (2026-09-13, OPEN): "icons" -- Hawai‘i's icons as a register, then a symbol family, then stories. Active / Exploring. 34 rows in the register after the opening session.**

**Opened and populated 2026-09-13.** Test: a Hawai‘i icon is an object, place, sound, word or practice that a person with no caption resolves to "Hawai‘i" and nothing else -- now scored as two columns, `distinct` (does it resolve to nothing else) and `recognized` (who knows it), after loco moco broke the single test. Classification: channel (visual / auditory / lexical / kinetic / gustatory / olfactory-provisional) x origin (native_hawaiian / missionary_monarchy / plantation_immigrant / local / territorial_tourist / military / natural / external). Three origin values (`external`, `missionary_monarchy`, `local`) were added by Kim's ruling during the session. Register: 20 pass, 12 borderline, 2 deliberate fails. Emerging finding: the borderlines all fail by a confusable Hawai‘i once owned and lost (surfboard, steel guitar, poke bowl, tiki) -- the density claim may split into *originated here* vs *still owned here*. Source of record `Projects\icons` (bucket 2): `icon_register.csv` + `README.md` codebook. Track: R on Kim's machine.

**Previous focus (2026-09-05, CLOSED): sake_brewery -- PUBLISHED.** Block preserved in `logs/focus_history.md`.

**⚠ CARRIED (in `deferred.md`):** none new. The commit-side stale-copy finding (session_log 2026-09-05) still awaits a number in `pkm_findings.md`. `C:/temp` scratch still flagged by `pkm_health()`.

**NEXT: more candidate batches (resident quadrant thin), then the first symbol: Diamond Head.**
<!-- /active-focus -->
...
| proj_icons.md | icons | Content project, data-driven (register → symbols → stories) | **NEW 2026-09-13 -- ACTIVE FOCUS. Active / Exploring.** Hawai‘i icons: caption-free "Hawai‘i and nothing else" test; channel × origin classification; outsider vs resident sets; density hypothesis. Track R on Kim's machine. Source of record `Projects\icons` (bucket 2): register 34 rows + codebook | 2026-09-13 |
