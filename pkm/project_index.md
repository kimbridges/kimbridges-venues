...
**Active Focus (2026-08-31, close): "Smart_Car" -- CH.5 AND CH.6 ARE DRAFTED, AND THE BOOK RENDERS END TO END FOR THE FIRST TIME.**

**Seven pieces stand: Foreword, ch.1-ch.6 -- about 15,900 words.** Full detail for this session is in `session_log.md` 2026-08-31; Kim's four accounts plus the calibration, tree-tunnel and ch.6-spine rulings are verbatim in `logs/proj_Smart_Car_log.md`.

**★★★ CH.5 `long_days.qmd` -- ~3,575 words, four tables.** Spine: **not one of the long days was about driving.** ★ **KIM RULED THE CALIBRATION STORY IS ITS BASELINE**, so the chapter now opens with it: mainland driving was ONE route, LAX to LSM, **exactly 100 miles**, and four rules were set with no experience behind them (one driver, a very long way, no night driving, no unfamiliar rush hour). **★★ THERE IS NO LEARNING CURVE -- first three logged trips median 301 mi, everything after 305, p = 0.44.** The calibration happened on the 2010 crossing, which has no trip log, so **the record opens already knowing the answer.** It was never stamina; the rules set the day. **A long day is not a hard day, it is a day one of the rules was suspended for.** ★ **Nancy picked the end city in real time from Honolulu off a GPS track; Kim's word is LOGISTICS, not navigation, and that role has no column.**

**★★★ THE SCAFFOLD'S EIGHT WAS MISSING THE LONGEST DAY: 2016-06-19 Richmond MO -> Denver, 640 mi, 58.6 mph gross.** Kim's reason inverts the obvious one -- his brother lives in Richmond, Mount Evans was hope not motive, and he drove it to get a dull stretch behind him. ★ **MOUNT EVANS IS NOW MOUNT BLUE SKY**; applied to ch.4 and re-rendered.

**★★★ THE BELLINGHAM CONFLICT WAS SETTLED BY A NUMBER NOBODY HAD TRANSCRIBED.** `TwoRed_fuel_timetemp.csv` held 3 rows for 2013 against 40 for 2016; 22 rows entered from the Alaska scan, every odometer key gated against the fuel log, **0 unmatched**. The day splits **122 mi in 6.05 h (20.2 mph), then 48.2, then 52.9** -- the sheet recorded the roll-off, Kim remembers the driving. ⚠ **He has not confirmed that reading.** ★★ **A conflict between two records was only a conflict because a third had never been read.**

**★★★ CH.6 `the_roads.qmd` -- ~1,720 words and the book's first figure built as an ARGUMENT.** Kim ruled the spine: *"The architects of the Interstate system brought a kind of uniformity: excellent highways. That was very important. Nature gives these highways character. It's equally important."* **`@fig-roads` puts both answers on ONE shared axis** -- median gross mph per trip (28.3 coastal to 47.6 plains interstate) above, every Deep South and Appalachian leg below -- **so the eye runs down one scale and watches separation become overlap. ONE HUE deliberately:** the groups are separated by row position, not colour, because two hues would assert a distinction the data cannot make. Group definitions are now **stated in code** and reproduce 40.3 / 38.0 / p = 0.42 exactly; they had never been written down. ★★ **The chapter found its own tie back to ch.5** -- the uniformity he could not wait to escape is the same specification that made the 305-mile day possible.

**★★ ONE EXAMPLE ONLY: I-65 north in Alabama, dated 2014-05-07/08.** Four other candidates CLOSED WON'T DO -- *a second example adds weight without adding argument*. **⛔ NO BLAME: Alabama is not at fault.** The category is **SIGHTLINES, not trees** -- a conjunction of three, with the western counter-case inside the definition; the term came from his nephew on the Siberia-to-Europe train.
