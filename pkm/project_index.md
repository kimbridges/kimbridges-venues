...
**Active Focus (2026-08-03, close): "PKM" -- LEGACY CLEANUP COMPLETE, AND THE SOURCE IS NOW BACKED UP.** *Experiments with LLMs* converted from its live Google Doc and published as the 22nd document, closing the last non-historical legacy link in any venue (3 remain: one historical mention kept by rule, two stale PROSE claims). **Then the consolidation migration was DELIVERED:** github.com/kimbridges/kimbridges-venues, 882 files / 5.8 MB, verified from GitHub's trees API -- 197 venue source + 683 project source across 49 projects, zero binaries or derived output. Before today the source for all 79 published works existed in exactly ONE place. **Three corrections made against evidence:** a mirror rather than `.git` on Drive (bucket 5 stands); a WHITELIST rather than an ignore list (the blacklist missed `_output/` and admitted 2018 files of Jones data); and `Projects/` included, because the documents venue holds rendered output whose real source lives there -- venues alone would back up the shelf and not the books. **The first push was REFUSED by GitHub push protection, which is the best thing that happened:** a live Anthropic API key had sat in `Projects/LLM_API_R/Claude_API.qmd` since April 2024, commented out with `##`. Never published (verified three ways). Removed; `pkm_secret_scan()` now gates every commit. Findings 027 (a public repo checks what a private disk never does) and 028 (three silent failures -- `git_push` reporting success while transferring nothing; a stale size read; the expensive-synced-read mistake made TWICE in one session). **Refresh cadence SETTLED:** `pkm_backup()` is Mechanism 5 item 12, running at every session close beside `pkm_health()` -- secret scan as a hard gate, push verified against the server. **Next up:** Kim to revoke the April-2024 key; the per-project credential audit of Drive; six repo/clone mismatches. Then the two stale prose claims and the three orphan story covers.
...
| proj_kimbridges_stories.md | kimbridges-stories | Website (Netlify) | Live; 51 stories; **Kaka‘ako Birds ("The Birds Around Kaka‘ako") PUBLISHED to the Stories site 2026-06-23 — the collaborative photo-book shipped (parked 2026-06-07 → unparked on McClatchey's read → live)** | 2026-06-23 |
...
| proj_AI_Podcasts.md | AI_Podcasts | Quarto multi-chapter document | **PUBLISHED 2026-08-02** to kimbridges-documents/ai_podcasts as the 21st document; eleven NotebookLM audio experiments. Source moved off the OneDrive Desktop the same day. **All audio now self-hosted — SoundCloud carries none of Kim's audio.** Open: four ISSUU pointers in Travel.qmd, blocked on locating the booklet PDFs. | 2026-08-02 |
